...
PublicVPNList 默认作为额外来源启用，可按国家页、速度、延迟、协议与 Technical score 过滤节点。下载到的 `.ovpn` 会在通过基本有效性与节点匹配检查后接入节点池；Playwright/Chromium 仅作为可选备用组件。
...
PUBLICVPNLIST_REQUIRE_REAL_DOWNLOAD=1  # 只接入通过校验的真实 .ovpn
PUBLICVPNLIST_PLAYWRIGHT_FALLBACK=1 # 启用可选备用下载组件
...
```
修改后执行 `ml restart` 生效。安装器会优先复用系统已有的新版 Python 创建 `/opt/aimilivpn/.venv`，Playwright Chromium 为可选备用依赖，非 apt 系统不会因备用浏览器依赖失败而阻断部署。OpenVPN 配置会写入节点 `.ovpn` 文件并交给 `OPENVPN_CMD --config` 使用；连接失败、进程退出或本地代理健康检查失败时，当前 IP 会被标记不可用并自动切换到备用节点。号池低于 `TARGET_VALID_NODES=3` 时会按 `LOW_POOL_RETRY_SECONDS` 补池；健康时按每日周期刷新。Technical score 是 PublicVPNList 的技术质量分，不等同于隐私或风控绝对保证。
...
PublicVPNList is enabled as an extra source by default. It can filter nodes by country page, speed, latency, protocol, and Technical score. Downloaded `.ovpn` profiles are added to the node pool after basic validity and node-match checks; Playwright/Chromium is only an optional fallback component.
...
PUBLICVPNLIST_REQUIRE_REAL_DOWNLOAD=1  # accept only verified real .ovpn profiles
PUBLICVPNLIST_PLAYWRIGHT_FALLBACK=1 # enable optional fallback downloader
...
TARGET_VALID_NODES=3
```
Run `ml restart` after changing these values. The installer reuses a newer system Python when available to create `/opt/aimilivpn/.venv`; Playwright Chromium is optional fallback dependency, and non-apt systems are not blocked by browser dependency failures. Downloaded profiles are written as node `.ovpn` files and passed to `OPENVPN_CMD --config`. If a connection fails, the OpenVPN process exits, or the local proxy health check fails, the active IP is marked unavailable and the manager switches to a backup node. When the pool falls below `TARGET_VALID_NODES=3`, the daemon refills it after `LOW_POOL_RETRY_SECONDS`; otherwise it refreshes daily. Technical score is PublicVPNList's technical quality score, not an absolute privacy or risk guarantee.
