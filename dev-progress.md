...
- 已按用户要求把生产下载模式改为 HTTP-first，Playwright 只保留为备用方式：`fetch_publicvpnlist_ovpn_config()` 现在优先使用纯 HTTP 会话进入 `/download/{data-id}/` 详情页，再请求 `/test_server.php?id=...` 做站点当前实时可达性检查，只有返回 `ok` 才请求 `/get_token.php?id=...` 生成一次性下载地址，并用 `/download.php?token=...` 下载真实 `.ovpn`；一次性 token 不写入节点、日志或结果文件。
- HTTP-first 原理：复用 PublicVPNList 前端按钮背后的接口链路，避免启动浏览器；服务端实时检查失败（例如不可达、连接超时、UDP quick check inconclusive）时不进入下载成功计数；下载后的内容必须通过 `looks_like_openvpn_config()`，并通过 `publicvpnlist_config_matches_item()` 校验 `.ovpn` 内 `remote host port/proto` 与当前国家页条目一致，否则丢弃并可进入备用链路。
- Playwright 备用原理：当 HTTP 接口、token 或下载阶段未取得可用 `.ovpn` 且 `PUBLICVPNLIST_PLAYWRIGHT_FALLBACK=1` 时，才启动 Chromium，限定点击当前详情页 `#downloadCurrentCheckBtn` 与 `#dlStart`，读取 `#dlReadyLink` 后下载，避免误点 Similar checked servers；如果 `/test_server.php` 已明确返回非 `ok`，该条目直接按当前不可用处理，不再启动浏览器备用链路；若备用链路失败则返回空配置并跳过该条目。
